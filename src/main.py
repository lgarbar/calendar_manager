...
URL mode uses Playwright; text/HTML modes require only the standard library.
"""
...
def load_url(url, profile_dir, headless=False, timeout_ms=120000):
    try:
        from playwright.sync_api import sync_playwright, TimeoutError as PlaywrightTimeoutError
    except ImportError:
        raise SystemExit("URL mode requires Playwright. Install it with: uv add playwright && uv run playwright install chromium")
    profile_dir = Path(profile_dir).expanduser().resolve()
    profile_dir.mkdir(parents=True, exist_ok=True)
    print(f"Opening {url}")
    print(f"Browser session profile: {profile_dir}")
    with sync_playwright() as p:
        context = p.chromium.launch_persistent_context(
            user_data_dir=str(profile_dir), headless=headless, viewport={"width": 1440, "height": 1000}
        )
        page = context.pages[0] if context.pages else context.new_page()
        page.goto(url, wait_until="domcontentloaded", timeout=timeout_ms)
        selector = "#enrolledClassSections_content .classAbbreviation"
        try:
            page.wait_for_selector(selector, state="attached", timeout=timeout_ms)
        except PlaywrightTimeoutError:
            if headless:
                context.close()
                raise SystemExit("The schedule did not load in headless mode. Re-run without --headless so you can sign in/MFA.")
            print("Schedule not detected yet. If Vanderbilt is asking you to sign in or complete MFA, do that in the opened browser window.")
            try:
                page.wait_for_selector(selector, state="attached", timeout=timeout_ms)
            except PlaywrightTimeoutError:
                context.close()
                raise SystemExit("Timed out waiting for the rendered schedule. Confirm the URL points to the Schedule page and that classes are visible.")
        raw = page.locator("#enrolledClassSections_content").evaluate("el => el.outerHTML")
        context.close()
...
def main():
    ap=argparse.ArgumentParser(description="Extract a class schedule from text, rendered HTML, or an authenticated URL and create an .ics calendar.")
    ap.add_argument("input", nargs="?", help="Schedule .txt/.html path or https:// URL. Omit to paste text interactively.")
    ap.add_argument("-o","--output",type=Path,default=Path("/data/test_data/output/class_schedule.ics"))
    ap.add_argument("--start")
    ap.add_argument("--end")
    ap.add_argument("--timezone",default="America/Chicago")
    ap.add_argument("--yes",action="store_true")
    ap.add_argument("--open",action="store_true",dest="open_file")
    ap.add_argument("--browser-profile",type=Path,default=Path.home()/".class-schedule-converter"/"chromium-profile",help="Persistent Playwright browser profile used to retain the Vanderbilt login session.")
    ap.add_argument("--headless",action="store_true",help="Run browser invisibly. Use only after login is already saved.")
    ap.add_argument("--browser-timeout",type=int,default=120,help="Seconds to wait for the rendered schedule/login (default: 120).")
    args=ap.parse_args()
    if args.input:
        if is_url(args.input):
            text=load_url(args.input,args.browser_profile,args.headless,args.browser_timeout*1000)
        else:
            path=Path(args.input).expanduser()
            if not path.exists(): raise SystemExit(f"Input file not found: {path}")
    ...
    if not classes: raise SystemExit("No courses found in the supplied schedule.")
